...
```
Micrófono Quest 2 → Whisper (STT local detecta inglés) → LLM (recibe prompt en inglés) → Respuesta texto inglés → TTS → Audio en NPC dentro de Godot XR
```
...
| **Text-to-Speech** | Piper TTS (local) o ElevenLabs (cloud) | Piper offline, requiere voces EN y ES |
...
| **Idioma del juego** | INGLÉS (NPCs y Jugador), ESPAÑOL (Ayudante y UI) | — |
| **Asistente de idioma** | LLM evaluador que corrige lo que dice el jugador (STT) | Si el STT saca "I is happy", el asistente interrumpe |
